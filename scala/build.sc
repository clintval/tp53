...
private val packageVersion = "0.11.0"
...
/** The seshat Scala package package. */
object seshat extends ScalaModule with PublishModule with ScoverageModule {
  ...
  def pomSettings: Target[PomSettings] = PomSettings(
    description    = "Scala tools for programmatically annotating VCFs with the Seshat TP53 database.",
    organization   = "io.cvbio",
    url            = "https://github.com/clintval/seshat",
    licenses       = Seq(License.MIT),
    versionControl = VersionControl.github(owner = "clintval", repo = "seshat", tag = Some(packageVersion)),
  ...
  override def artifactName: T[String] = T { "seshat" }
  ...
  def executable = T {
    ...
    println("Copying artifact to ./bin/seshat")
    copy(artifact.path, pwd / "bin" / "seshat", replaceExisting = true)
  }
  ...
  override def ivyDeps = Agg(
    ...
    ivy"io.cvbio.collection.mutable::bimap::2.0.0",
    ivy"io.cvbio.io::clitool::0.1.0",
    ivy"org.rogach::scallop::5.1.0",
    ivy"org.slf4j:slf4j-simple:1.7.6",
  )
